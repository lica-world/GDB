...
class BenchmarkRunner:
    ...
    def run(
        ...
        *,
        ...
        dataset_root : str or Path, optional
            Top-level dataset directory.  If ``None`` (and *data_dir* is
            also ``None``), samples are loaded from the HuggingFace Hub
            dataset ``lica-world/GDB`` instead.
        n : int, optional
            Limit samples per benchmark (passed to ``load_data``).
        batch_size : int, optional
            If set, call ``model.predict_batch()`` with chunks of this
            size instead of sequential ``predict()`` calls.
        ...
        use_hub = dataset_root is None and override_dir is None
        ...
        for bid in benchmark_ids:
            ...
            if use_hub:
                ...
                samples = load_from_hub(bid, n=n)
                logger.info("Loaded %d samples for %s from HuggingFace", len(samples), bid)
            else:
    ...
    def submit(
        ...
        *,
        ...
        if dataset_root is None and data_dir is None:
            from .hf import load_from_hub
            samples = load_from_hub(benchmark_id, n=n)
        else:
            resolved_dir = (
