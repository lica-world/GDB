...
class IntentToLayoutGeneration(BaseBenchmark):
    ...
    LAYOUT2_MANIFEST_ENV = "GDB_LAYOUT2_MANIFEST"
    ...
    def _mjudge_enabled(cls) -> bool:
        flag = os.environ.get("GDB_ENABLE_MJUDGE")
        configured_model = str(os.environ.get("GDB_MJUDGE_MODEL", "")).strip()
        if flag is None:
    ...
    def _load_mjudge_bundle(cls) -> Any:
        ...
        if ":" not in model_spec:
            logger.info("Invalid GDB_MJUDGE_MODEL=%r", model_spec)
            cls._mjudge_bundle = False
            return cls._mjudge_bundle
        ...
        kwargs: Dict[str, Any] = {
            "model_id": model_id,
            "temperature": 0.0,
            "max_tokens": max(32, cls._safe_int(os.environ.get("GDB_MJUDGE_MAX_TOKENS"), 128)),
        }
        credentials_path = str(os.environ.get("GDB_MJUDGE_CREDENTIALS", "")).strip()
    ...
    def _hpsv3_score(cls, prompt: str, image: np.ndarray, clip_fallback: float) -> float:
        if cls._hpsv3_bundle is None:
            try:
                ...
                preferred_device = os.environ.get("GDB_HPS_DEVICE")
    ...
    def _ensure_hpsv2_assets(cls, hpsv2_module: Any) -> None:
        ...
        try:
            ...
            req = Request(url, headers={"User-Agent": "gdb"})
...
class PartialLayoutCompletion(IntentToLayoutGeneration):
    ...
    def load_data(
        ...
        *,
        ...
        if has_single or has_multiple:
            mode_env = os.environ.get("GDB_PARTIAL_MODE", "all").strip().lower()
            if mode_env not in {"all", "single", "multiple"}:
                logger.warning(
                    "Invalid GDB_PARTIAL_MODE=%r. Falling back to 'all'.",
                    mode_env,
                )
                mode_env = "all"
    ...
    def evaluate(self, predictions: List[Any], ground_truth: List[Any]) -> Dict[str, float]:
        from gdb.metrics.core import iou as iou_metric
    ...
    def _maybe_save_layout3_renders(
        ...
        *,
        ...
        out_root_raw = str(os.environ.get("GDB_LAYOUT3_SAVE_RENDERS_DIR", "")).strip()
        if not out_root_raw:
            return
...
class AspectRatioAdaptation(PartialLayoutCompletion):
    ...
    PAIR_MANIFEST_ENV = "GDB_LAYOUT4_PAIR_MANIFEST"
    DIRECTION_ENV = "GDB_LAYOUT4_DIRECTION"
    DIRECT_EVAL_SIZE_ENV = "GDB_LAYOUT4_DIRECT_EVAL_SIZE"
